...
class NN(tfkl.Layer):
    ...
    '''
    def __init__(self, n_dim, n_layer=3, n_hid=512, window_size=16, activation='relu', name='fc_layer'):
        ...
        for _ in range(n_layer):
            self.layer_list.append(tfkl.LSTM(n_hid, input_shape=(window_size,n_dim,),activation=activation))
...
class TwoNVPCouplingLayers(tfkl.Layer):
    def __init__(self, inp_dim, n_hid_layer, n_hid_dim, name, shuffle_type, window_size=16):
        ...
        self.nn1 = NN(inp_dim, n_hid_layer, n_hid_dim, window_size=16)
        self.nn2 = NN(inp_dim, n_hid_layer, n_hid_dim, window_size=16)
    ...
    def split(self, x):
        ...
        x = tf.reshape(x, [-1, self.window_size, self.inp_dim])
        return x[:, :, :self.inp_dim//2], x[:, :, self.inp_dim//2:]


class NVP(tfk.Model):
    def __init__(self, inp_dim, n_couple_layer, n_hid_layer, n_hid_dim, name, shuffle_type='reverse', window_size=16):
        ...
        for i in range(n_couple_layer):
            layer = TwoNVPCouplingLayers(
                ...
                name=f'Layer{i}', shuffle_type=shuffle_type, window_size=window_size)
...
if __name__ == "__main__":
    ...
    model = NVP(inp_dim, n_couple_layer, n_hid_layer, n_hid_dim, name='NVP', window_size=window_size)
    x = tfkl.Input(shape=(window_size, inp_dim,))
    model(x)
    model.summary()

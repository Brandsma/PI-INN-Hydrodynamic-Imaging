...
def main():
    ...
    new_model = tf.keras.models.load_model(
        './trained_models/win16_stride2_epochs10_latest')
    new_model.summary()
    prev_x = 0
    prev_time = 0
    prev_x_label = 0
    ...
    for idx in tqdm(range(0, 1024, window_size)):
        ...
        if idx != 0:
            ...
            speed = (y_pred[0][0] - prev_x) / (time - prev_time)
            real_speed = (x_label - prev_x_label) / (time - prev_time)

            # print("real loc1:", x_label)
            # print("real loc0:", prev_x_label)
            # print("loc1:", y_pred[0][0])
            # print("loc0:", prev_x)
            # print("delta meter:", (y_pred[0][0] - prev_x), "delta seconds:",
            #       (time - prev_time))
            # print("Speed:", speed)
    ...
    plt.text(0, 6, f"MSE: {MSE:.2f}")
    plt.title("Estimated vs Real speed per sensor reading window")
    plt.legend()
    plt.show()

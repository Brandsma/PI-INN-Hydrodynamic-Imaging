# Required libraries
from tensorflow.keras.layers import LSTM, Dropout, Masking, Activation
...
class LSTM_network:
    ...
    def __init_network(self):
        ...
        model = Sequential()
        # with an LSTM layer
        model.add(LSTM(n_nodes, input_shape = (win_size, n_inputs), activation = self.activation))
        ...
        optimizer = optimizers.Adagrad(lr = alpha, epsilon=None, decay = decay, clipnorm = 1.)
    ...
    def test(self, data, labels):
        ...
        if hasattr(self, 'hist'):
            ...
            for lab_idx in range(0, len(labels)):
                ...
                if lab_idx%10 == 0:
                    plt.plot(y_pred)
                    plt.plot(y_true)
                    plt.legend(['x_pred', 'y_pred', 'x_true', 'y_true'], loc = 'upper left')
                    # plt.ylim(-35, 35)
        ...
        # Otherwise give an error    
            ...
            print ("Run the network by calling the \'.train()\' method on the LSTM object.\n")
            ...
            for i in range(0, len(data)):
                ...
                for idx in range(0, len(sample) - window_size + 1, stride):
                    while batch_idx > (self.batch_size - 1):
                        ...
                        batch_idx = 0
                    batch_idx += 1
                    x = np.vstack((x, np.reshape(sample[idx: idx + window_size], (1, window_size, self.data.n_inputs))))
                    y = np.vstack((y, np.reshape(sample_lab[idx + window_size-1: idx + window_size], (1, self.data.n_outputs))))
                yield (x, y)
    ...
    def save_results(self):
        ...
        np.savetxt(dirname + "/" + "val_loss.out", self.hist.history['val_loss'])

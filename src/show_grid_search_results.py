...
def main():
    ...
    for model_type in model_types:
        ...
        if model_type != "lstm":
            ...
            inn_df["localization_error"] = (inn_df["localization_error"] * 0.1).round(2).astype(str) + " (±" + (abs(inn_df["localization_error_std"]) * 0.1).round(2).astype(str) + ")"
            inn_df["volume_error"] = (inn_df["volume_error"] * 0.1).round(2).astype(str) + " (±" + (abs(inn_df["volume_error_std"])* 0.1).round(2).astype(str) + ")"
            inn_df["speed_error"] = (inn_df["speed_error"] * 0.1).round(2).astype(str) + " (±" + (abs(inn_df["speed_error_std"]) * 0.1).round(2).astype(str) + ")"
        else:
            inn_df["localization_error"] = inn_df["localization_error"].round(2).astype(str) + " (±" + abs(inn_df["localization_error_std"]).round(2).astype(str) + ")"
            inn_df["volume_error"] = inn_df["volume_error"].round(2).astype(str) + " (±" + abs(inn_df["volume_error_std"]).round(2).astype(str) + ")"
            inn_df["speed_error"] = inn_df["speed_error"].round(2).astype(str) + " (±" + abs(inn_df["speed_error_std"]).round(2).astype(str) + ")"
        ...
        print(tabulate(inn_df, headers="keys", tablefmt="latex_longtable", showindex=False))
        print('\n')

...
class LSTMTester:
    def __init__(
        self,
        model_folder="../data/trained_models/LSTM/",
        model_name=None,
        result_path="../data/results/LSTM/",
        save_to_file=True,
        debug=False,
    ):
        # TODO: path maybe?
        ...
        self.model = tf.keras.models.load_model(f"{model_folder}{self.model_location}")
        self.debug = debug
    ...
    def set_input_data(self, a, w, simulation_subset):
        ...
        # Load settings
        self.settings = params.Settings.from_model_location(
            self.model_location, self.train_location
        )
        # Load data
    ...
    def get_data(self):
        ...
        for run_idx in range(0, len(self.data.test_labels)):
            ...
            speed, _ = get_speed_from_model_predicts(
                ...
                self.data.test_timestamp[
                    run_idx
                ],  # NOTE: Taking a random timestamp, since they should all be the same
    ...
    def __predict_data(self, run_idx):
        ...
        for idx in range(
            0, len(self.data.test_data[run_idx]) - self.settings.window_size
        ):
            ...
            true_label = self.data.test_labels[run_idx][
                idx + self.settings.window_size - 1 : idx + self.settings.window_size
            ][0]

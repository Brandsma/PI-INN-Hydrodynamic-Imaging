...
def test_lstm():
    ...
    for num_sensors in sensor_options:
        ...
        model = tf.keras.models.load_model(
            ...
            else f"../data/trained_models/noise/LSTM/{model_location}"
        )
        for subset in subsets:
            ...
            # Load settings
            settings = params.Settings.from_model_location(
                model_location, train_location
            )
            ...
            hydro.plot_results_from_array(
                ...
                num_sensors,
                savefig=True,
                title=f"{subset} | {num_sensors}",
                savepath=results_folder,
            )

            print("Saving the following files:")
            print(
                data.test_data.reshape(-1, num_sensors * 2).shape,
                data.test_labels.reshape(-1, 3).shape,
                network.pred.shape,
            )
            np.save(
                results_folder + f"/x_data_{num_sensors}.npy",
                data.test_data.reshape(-1, num_sensors * 2),
            )
            np.save(
                results_folder + f"/y_data_{num_sensors}.npy",
                data.test_labels.reshape(-1, 3),
            )
            np.save(
                results_folder + f"/y_pred_{num_sensors}.npy", network.pred
            )  # .reshape(-1, 3))

...
if __name__ == "__main__":
    ...
    x_train = np.concatenate((data.train_data, data.train_labels), axis=2).reshape(
        (-1, data.train_data.shape[2] + data.train_labels.shape[2])
    )
    y_train = np.repeat(data.train_volumes.reshape((-1, 1)), 1024, axis=1).reshape(
        (-1, 1)
    )
    # train_data = tf.data.Dataset.from_tensor_slices((x_train, y_train))

    x_val = np.concatenate((data.val_data, data.val_labels), axis=2).reshape(
        (-1, data.val_data.shape[2] + data.val_labels.shape[2])
    )
    y_val = np.repeat(data.val_volumes.reshape((-1, 1)), 1024, axis=1).reshape((-1, 1))

    x_test = np.concatenate((data.test_data, data.test_labels), axis=2).reshape(
        (-1, data.test_data.shape[2] + data.test_labels.shape[2])
    )
    y_test = np.repeat(data.test_volumes.reshape((-1, 1)), 1024, axis=1).reshape(
        (-1, 1)
    )
    ...
    model.add(
        tf.keras.Input(shape=(data.train_data.shape[2] + data.train_labels.shape[2]))
    )
    ...
    model.compile(
        ...
        metrics=[tf.keras.metrics.MeanSquaredError(), tf.keras.losses.LogCosh()],
    )
    ...
    for run_idx in tqdm(range(data.test_data.shape[0])):
        ...
        x_test = np.concatenate(
            (data.test_data[run_idx], data.test_labels[run_idx]), axis=1
        ).reshape((-1, data.test_data.shape[2] + data.test_labels.shape[2]))

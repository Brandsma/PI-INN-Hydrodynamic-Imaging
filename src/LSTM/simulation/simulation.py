...
def v_x(s, x, y, theta, a, norm_w):
    p = (s - x) / y
    C = (norm_w * a**3) / (2 * y**3)
    return C * (wavelet_o(p) * math.sin(theta) - wavelet_e(p) * math.cos(theta))


def v_y(s, x, y, theta, a, norm_w):
    p = (s - x) / y
    C = (norm_w * a**3) / (2 * y**3)
    return C * (wavelet_n(p) * math.sin(theta) + wavelet_o(p) * math.cos(theta))
...
def calculate_path(points, num_steps, simulation_area_offset=75):
    ...
    for idx in range(len(points) - 1):
        ...
        y_input = list(
            np.linspace(
                points[idx][1] + simulation_area_offset,
...
def simulate_single_step(
    input_sensors, x, y, theta, a, norm_w, add_noise=True, noise_power=1.5e-5
):
    data = []
...
def simulate(
    ...
    path = calculate_path(
        points, number_of_steps, simulation_area_offset=simulation_area_offset
    )
    ...
    for _ in tqdm(range(number_of_runs)):
        ...
        for path_idx, (x, y) in enumerate(path):
            ...
            for input_sensor in input_sensors:
                # NOTE: the x and y coordinates are different than the array coordinates
                data[path_idx].append(v_x(input_sensor, x, y + 1, theta, a, norm_w))
                data[path_idx].append(v_y(input_sensor, x, y + 1, theta, a, norm_w))
            ...
            if add_noise:
                data[path_idx] += np.random.normal(0, noise_power, len(data[path_idx]))

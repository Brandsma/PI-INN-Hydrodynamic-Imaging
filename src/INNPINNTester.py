...
class INNPINNTester():
    ...
    def set_input_data(self, a, w, simulation_subset):
        ...
        self.train_data, self.train_labels, self.test_data, self.test_labels = setup_data_with_data(deepcopy(self.unnormalized_data))
    ...
    def get_data(self):
        if not hasattr(self, 'settings') or not hasattr(self, 'test_labels') or not hasattr(self, 'network') or not hasattr(self, 'config'):
        ...
        for run_idx in range(0, len(self.test_labels)//1024):
            ...
            volume = extract_volume(x_pred[:, :self.x_dim],
                                    speed,
                                    vx_data,
                                    vy_data,
                                    self.test_labels[(1024 * run_idx):(1024 + 1024*run_idx)],

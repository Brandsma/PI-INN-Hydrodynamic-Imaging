...
def table_data(dt="speed", noise_experiment=False):
    ...
    for subset in subsets:
        ...
        for idx, model in enumerate(models):
            with open(Path(f"../../results/{dt}_{model}_{subset}_results.json")) as f:
                results = json.load(f)
            ...
            if f"{subset}" in best_model_idx and best_model_idx[f"{subset}"] == idx:
                if dt != "volume" and dt != "speed":
                    s.append(
                        ...
                        + " (±"
                        + f"{results['combined'][1]:.2f}"
                        + ")}"
                    )
                else:
                    s.append("\\textbf{" + f"{results['combined'][0]:.2f}" + "}")
            else:
                if dt != "volume" and dt != "speed":
                    s.append(
                    ...
                    s.append(f"{results['combined'][0]:.2f}")
        df = pd.concat([df, pd.Series(s, name=model)], axis=1)
    df.rename(
        {x: translation_key[subsets[x]] for x in range(len(subsets))},
        axis=0,
        inplace=True,
    )
    ...
    print(
        df.style.to_latex(
            ...
            hrules=True,
            caption="Results for \\textbf{"
            + dt
            + "} estimation. The mean and standard deviation of the combined RMS error over 25 runs is shown for each model and path. The units are in degrees. The best mean per path is shown in bold, lower is better.",
            label=f"tab:{dt}_MSE_results",
        )
    )

...
def sensor_difference_visualization():
    for num_sensors in [8, 64]:
        ...
        data = simulate_single_step(input_sensors, x=0, y=75, theta=0, a=10, norm_w=10, add_noise=True, noise_power=1.5e-5)
        ...
        plot_velocity_profiles(input_sensors, vx, vy, title=f"Velocity Profiles for {num_sensors} Sensors")
    plt.show()
...
def speed_difference_visualization():
    ...
    for a in reversed(a_list):
        ...
        data = simulate_single_step(input_sensors, x=0, y=75, theta=0, a=10, norm_w=a, add_noise=True, noise_power=1.5e-5)

        vx = data[0::2]
        vy = data[1::2]

        plot_velocity_profiles(input_sensors, vx, vy, title=f"$v_y$ for Various Speeds", plot_vx=False, vy_label=r"$\left\Vert W \right\Vert={}$".format(a), vy_color=color_gradient.pop().hex_l)
    plt.ylim((-0.15,0.15))
    plt.xlim((-200, 200))

    plt.savefig("../../Thesis/images/speed_difference_visualization_vy.pdf", dpi=300, bbox_inches="tight")
    ...
    for a in reversed(a_list):
        ...
        plot_velocity_profiles(input_sensors, vx, vy, title=f"$v_x$ for Various Speeds", plot_vy=False, vx_label=r"$\left\Vert W \right\Vert={}$".format(a), vx_color=color_gradient.pop().hex_l)
    plt.ylim((-0.15,0.15))
    plt.xlim((-200, 200))

    plt.savefig("../../Thesis/images/speed_difference_visualization_vx.pdf", dpi=300, bbox_inches="tight")
...
def volume_difference_visualization():
    ...
    for a in reversed(a_list):
        input_sensors = np.linspace(-200, 200, 64)
        data = simulate_single_step(input_sensors, x=0, y=75, theta=0, a=a, norm_w=10, add_noise=True, noise_power=1.5e-5)

        vx = data[0::2]
        vy = data[1::2]

        plot_velocity_profiles(input_sensors, vx, vy, title=f"$v_y$ for Various Volumes", plot_vx=False, vy_label=r"$a={}$".format(a), vy_color=color_gradient.pop().hex_l)
    plt.ylim((-2,2))
    plt.xlim((-200, 200))

    plt.savefig("../../Thesis/images/volume_difference_visualization_vy.pdf", dpi=300, bbox_inches="tight")
    ...
    for a in reversed(a_list):
        ...
        plot_velocity_profiles(input_sensors, vx, vy, title=f"$v_x$ for Various Volumes", plot_vy=False, vx_label=r"$a={}$".format(a), vx_color=color_gradient.pop().hex_l)
    plt.ylim((-2,2))
    plt.xlim((-200, 200))


    plt.savefig("../../Thesis/images/volume_difference_visualization_vx.pdf", dpi=300, bbox_inches="tight")


def plot_velocity_profiles(input_sensors, vx, vy, title="Velocity Profiles", plot_vx=True, plot_vy=True, vx_label="$v_x$", vy_label="$v_y$", vx_color="#2A9D8F", vy_color="#E76F51"):
    if plot_vx:
...
def contour_plot():
    ...
    data = simulate_single_step(input_sensors, x=X, y=Y, theta=0, a=30, norm_w=30, add_noise=True, noise_power=1.5e-5)

...
# Code from https://github.com/otenim/TensorFlow-OS-ELM.
...
class OS_ELM(object):
    ...
    def __init__(
        ...
        self.__x = tf.placeholder(
            tf.float32, shape=(None, self.__n_input_nodes), name="x"
        )
        self.__t = tf.placeholder(
            tf.float32, shape=(None, self.__n_output_nodes), name="t"
        )
        ...
        self.__predict = tf.matmul(
            self.__activation(tf.matmul(self.__x, self.__alpha) + self.__bias),
            self.__beta,
        )
        ...
        self.__accuracy = tf.reduce_mean(
            tf.cast(
                tf.equal(tf.argmax(self.__predict, 1), tf.argmax(self.__t, 1)),
                tf.float32,
            )
        )
    ...
    def evaluate(self, x, t, metrics=["loss"]):
        ...
        for m in metrics:
            if m == "loss":
                ...
                return ValueError("an unknown metric '%s' was given." % m)

...
class INNTester():
    ...
    def set_input_data(self, a, w, simulation_subset):
        ...
        # Load settings
        self.settings = params.Settings(shuffle_data=False,
                                        train_location=self.train_location)

        # Load data
        self.unnormalized_data = params.Data(self.settings,
                                             self.train_location)
        self.train_data, self.train_labels, self.test_data, self.test_labels = setup_data_with_data(
            deepcopy(self.unnormalized_data))

        # Load model
        model = create_model(self.tot_dim, self.config.n_couple_layer,
                             self.config.n_hid_layer, self.config.n_hid_dim)
    ...
    def get_data(self):
        if not hasattr(self, 'settings') or not hasattr(
                self, 'test_labels') or not hasattr(
                    self, 'network') or not hasattr(self, 'config'):
            print(
                "Make sure to run 'set_input_data' before trying to retrieve data!"
            )
            return
        ...
        for run_idx in range(0, len(self.test_labels) // 1024):
            ...
            result_data.append(
                (x_pred, y_pred,
                 mean_squared_error(y_data[:, :self.y_dim],
                                    y_pred[:, :self.y_dim]),
                 mean_squared_error(x_data[:, :self.x_dim],
                                    x_pred[:, :self.x_dim]), volume, speed))
    ...
    def save_result_data(self):
        if not hasattr(self, "result_data"):
            print(
                "First run get_data and set 'result_data' before trying to save it"
            )

        np.save(
            Path(
                f"{self.result_path}{self.simulation_subset}_a{self.a}_w{self.w}.npy"
            ), self.result_data)
...
def generate_inn_options():
    ...
    return cartesian_coord(n_couple_layer_options, n_hid_layers_options,
                           n_hid_dim_options, z_dim_options)
